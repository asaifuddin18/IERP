...
async def _redeemCode(ctx, name: str):
    await ctx.respond(eat=True)
    #await ctx.send("Redeemed Code!")
    username = await client.fetch_user(ctx.author_id)
    user_id = int(ctx.author_id)
    if name in active_codes.keys() and user_id in df["ID"].values: #valid key AND old user

        if not used.at[used.index[used["ID"] == user_id][0], name]: #make sure code has not already been redeemed!
            print("old user")
            index = df.index[df["ID"] == user_id][0]
            df.Points[index] = df.Points[index] + active_codes[name][0]
            used.at[used.index[used["ID"] == user_id], name] = True #marked redeemed
    ...
    elif name in active_codes.keys(): #valid key AND new user
        print("new user")
        df.loc[len(df.index)] = [user_id, username, active_codes[name][0]]
        temp  = [user_id]
        for i in range(len(used.columns) - 1):
            temp.append(False)
        
        used.loc[len(used.index)] = temp
        used.at[used.index[used["ID"] == user_id], name] = True
        await ctx.send("Code redeemed!", hidden=True)
        print(df)
        print(used)
...
def expired():
    while True:
        ...
        for code in active_codes.keys():
            if active_codes[code][1] + active_codes[code][2] < time.time():
                print(code, "expired")
                del active_codes[code]
                break

        df.to_csv('points.csv', index=False)
        used.to_csv('used.csv', index=False)


thread = Thread(target = expired)
thread.daemon = True
thread.start()
client.run(os.getenv('IERP'))
